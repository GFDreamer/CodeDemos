...
@property (strong,nonatomic) MLPhotoTipsView *tipsView;
@end
...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        rightBtn.frame = CGRectMake(0, 7.5, 70, 30);
...
- (MLPhotoTipsView *)tipsView
{
    if (!_tipsView)
    {
        _tipsView = [[MLPhotoTipsView alloc] init];
         __weak typeof(self) weakSelf = self;
        [_tipsView setBlock:^()
        {
            [weakSelf.navigationItem.rightBarButtonItem setTitle:@"照片说明"];
        }];
    }
    return _tipsView;
}
...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets{
    ...
    self.makeView.text = [NSString stringWithFormat:@"%@",@(count)];
    [self.doneBtn setTitle:@"确定" forState:UIControlStateNormal];
    if (count > 0)
    {
       [self.doneBtn setTitle:[NSString stringWithFormat:@"确定(%@)",@(count)] forState:UIControlStateNormal];
    }
...
#pragma mark 初始化按钮
- (void) setupButtons{
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"照片说明" style:UIBarButtonItemStyleDone target:self action:@selector(back)];
...
//    [self dismissViewControllerAnimated:YES completion:nil];
    if ([self.navigationItem.rightBarButtonItem.title isEqualToString:@"收起说明"])
    {
        self.navigationItem.rightBarButtonItem.title = @"照片说明";
        [self.tipsView removeFromSuperview];
    }
    else
    {
        self.navigationItem.rightBarButtonItem.title = @"收起说明";
